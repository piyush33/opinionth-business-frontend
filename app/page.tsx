// app/login/page.tsx  (your LoginPage with org flow)
"use client";
...
export default function LoginPage() {
  ...
  const orgSlugParam = search.get("org"); // workspace slug
  const inviteToken = search.get("t"); // invite token (org or layer)
  ...
  //   const res = await api.get(`/profileusers/${username}`);
  //   return res.data;
  // };
  ...
  const handlePostAuthOrgRouting = async (user: {
    ...
    // 1) Invite token in URL? Join first, then route.
    if (inviteToken) {
      try {
        const { data: preview } = await InviteApi.preview(inviteToken);
        // preview: { scope: 'org' | 'layer', organization: {id,slug,name}, layer?: {id,...} }
        await InviteApi.accept(inviteToken); // backend uses req.user / acceptorId as needed
        ...
        if (preview.scope === "org") {
          ...
          // NEW: set org context so org-scoped API calls work
          const org: OrgSummary = preview.organization;
        ...
        // fall through to normal membership handling
      }
    }

    // 2) org slug in URL? Validate membership and route if member.
    if (orgSlugParam) {
      try {
    ...
    // 3) No slug: show picker (many), auto-route (one), or prompt to create (none)
    const { data: memberships } = await OrgApi.membershipsForUser(
      user.username
    );
    const list = memberships as Membership[];
    if (list.length === 1) {
      routeToOrg(list[0].organization);
      return;
    }
    if (list.length > 1) {
      router.push(`/select-org`); // simple picker page (below)
      return;
    }
    ...
    if (user.email) {
      try {
        ...
        if (suggested.length) {
          // show a modal selector where they can "Join" or "Create"
    ...
    // 0 workspaces - show create workspace modal
