...
// API service
const createApiService = (getAuthHeaders: () => Record<string, string>) => ({
    ...
    postComment: async (cardId: number, username: string, image: string, comment: string) => {
        const response = await fetch(`${API_BASE_URL}/comments/${cardId}`, {
            method: "POST",
            headers: getAuthHeaders(),
...
export default function CardExpansionPage() {
    ...
    const apiService = useMemo(() => createApiService(getAuthHeaders), [getAuthHeaders])
        ...
        const fetchInteractionStates = async () => {
            ...
            try {
                ...
                const [likedResponse, savedResponse] = await Promise.allSettled([
                    axios.get(`${API_BASE_URL}/likes/homefeed/${userProfile.username}/${card.id}`, { headers }),
                    axios.get(`${API_BASE_URL}/saves/homefeed/${userProfile.username}/${card.id}`, { headers }),
                ])

                const likedResult = likedResponse.status === "fulfilled" ? likedResponse.value.data.hasLiked : false

                const savedResult = savedResponse.status === "fulfilled" ? savedResponse.value.data.hasSaved : false
        ...
        async (e: React.MouseEvent, item: CardData) => {
            ...
            try {
                if (!isCurrentlyLiked) {
                    // Like the item
                    await Promise.all([
                        axios.post(`${API_BASE_URL}/profilefeed/${userProfile.username}/liked`, item, { headers }),
                        axios.post(`${API_BASE_URL}/likes/homefeed/${userProfile.username}/${item.id}`, {}, { headers }),
                    ])
                } else {
                    // Unlike the item
                    await axios.delete(`${API_BASE_URL}/likes/homefeed/${userProfile.username}/${item.id}`, { headers })

                    // Also remove from profile liked feed
                    try {
                        const profileLikedResponse = await axios.get(`${API_BASE_URL}/profilefeed/${userProfile.username}/liked`, {
                            headers,
                        })
                        ...
                        if (likedItem) {
                            await axios.delete(`${API_BASE_URL}/profilefeed/${userProfile.username}/liked/${likedItem.id}`, {
                                headers,
                            })
                        }
                    } catch (error) {
    ...
    const handleSave = useCallback(
        async (e: React.MouseEvent, item: CardData) => {
            ...
            try {
                if (!isCurrentlySaved) {
                    await Promise.all([
                        axios.post(`${API_BASE_URL}/profilefeed/${userProfile.username}/saved`, item, { headers }),
                        axios.post(`${API_BASE_URL}/saves/homefeed/${userProfile.username}/${item.id}`, {}, { headers }),
                    ])
                } else {
                    await axios.delete(`${API_BASE_URL}/saves/homefeed/${userProfile.username}/${item.id}`, { headers })

                    // Also remove from profile saved feed
                    try {
                        const profileSavedResponse = await axios.get(`${API_BASE_URL}/profilefeed/${userProfile.username}/saved`, {
                            headers,
                        })
                        ...
                        if (savedItem) {
                            await axios.delete(`${API_BASE_URL}/profilefeed/${userProfile.username}/saved/${savedItem.id}`, {
                                headers,
                            })
                        }
                    } catch (error) {
            ...
            <main className="md:pt-20 bg-white">
                                ...
                                <>
                                    ...
                                    <div className="w-1/2 p-8 bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col justify-center">
                                        {card.text && (
                                            <div className="space-y-4">
                                                <div className="text-sm text-gray-500 uppercase tracking-wide font-medium">Content</div>
                                                <div className="prose prose-gray max-w-none">
                                                    <p className="text-gray-800 text-lg leading-relaxed whitespace-pre-wrap">{card.text}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                    <div className="w-1/2 flex flex-col">
                                        <div className="p-6 border-b border-gray-100 flex-shrink-0">
                                            <div className="flex items-start justify-between mb-4">
                                                <div className="flex-1">
                                                    <h1 className="text-2xl font-bold text-gray-900 mb-3">{card.title}</h1>
                ...
                <div className="md:hidden">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-4 space-y-4">
                                ...
                                <div className="flex items-center justify-between pt-2">
                                    <div className="flex items-center space-x-6">
                                        ...
                                        >
                                            <Heart className={`w-7 h-7 ${isLiked(card.id) ? "fill-red-500 text-red-500" : ""}`} />
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <button
                                            ...
                                            className="text-white hover:scale-110 transition-transform"
                                        >
                                            <Bookmark className={`w-6 h-6 ${isSaved(card.id) ? "fill-white" : ""}`} />
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-purple-50 to-blue-50 pt-16 pb-6">
                                <div className="p-4 space-y-4">
                                    ...
                                    <div className="flex items-center space-x-3 cursor-pointer" onClick={handleUserClick}>
                                        ...
                                        <div>
                                            <p className="font-semibold text-gray-900 text-base">{card.username}</p>
                                    ...
                                    <div className="space-y-3">
                                        <h1 className="text-gray-900 font-bold text-2xl leading-tight">{card.title}</h1>
                            ...
                            {card.text && (
                                <div className="p-4 bg-white">
                                    <div className="bg-gray-50 rounded-xl p-6">
                                        <div className="prose prose-gray max-w-none">
                                            <p className="text-gray-800 text-base leading-relaxed whitespace-pre-wrap m-0">{card.text}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="p-4 bg-white border-t border-gray-100 sticky bottom-0">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-6">
                                        ...
                                        <button onClick={handleShare} className="text-gray-700 hover:text-purple-500 transition-colors">
                                            <Share className="w-7 h-7" />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        >
                                            <Bookmark className={`w-6 h-6 ${isSaved(card.id) ? "fill-current" : ""}`} />
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="p-4  bg-white border-t border-gray-200">
                        <h3 className="text-black font-semibold text-lg mb-6 mt-6 text-center">More to explore</h3>
                        <MasonryGrid>
                            {feedCards.map((item) => (
                                <Card
                                    key={item.id}
