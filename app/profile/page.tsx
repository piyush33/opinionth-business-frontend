    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            ...
            try {
                ...
                const [createdRes, likedRes, savedRes, repostedRes, followersRes, followingRes] = await Promise.all([
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profilefeed/${user.username}/created`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profilefeed/${user.username}/liked`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profilefeed/${user.username}/saved`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profilefeed/${user.username}/reposted`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${user.username}/followers`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${user.username}/following`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                ])

                const [created, liked, saved, reposted, followersData, followingData] = await Promise.all([
                    createdRes.json(),
                    likedRes.json(),
                    savedRes.json(),
                    repostedRes.json(),
                ...
                setLikedPosts(liked)
                setSavedPosts(saved)
                setRepostedPosts(reposted.map((item: any) => ({ ...item, reposted: true })))
                ...
                <div className="max-w-6xl mx-auto">
                            ...
                            <div className="flex-1 text-center lg:text-left w-full">
                                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                                    <div>
                                        ...
                                        {user.tagline && <p className="text-gray-700 mb-4">{user.tagline}</p>}
                                    </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <div className="text-gray-400 text-lg mb-2">No {activeTab} posts yet</div>
