...
export default function ChatInterface({ selectedUser }: ChatInterfaceProps) {
    ...
    const fetchMessages = async () => {
        ...
        try {
            const response = await axios.get(
                `https://d3kv9nj5wp3sq6.cloudfront.net/messages/conversation/${user.username}/${selectedUser.username}`,
            )
    ...
    const sendMessage = async () => {
        ...
        try {
            await axios.post(`https://d3kv9nj5wp3sq6.cloudfront.net/messages/${user.username}/${selectedUser.username}`, {
                content: message,
            })
                ...
                {isLoading ? (
                    ...
                    <>
                        {messages.map((msg, index) => (
                            ...
                            >
                                <div
                                    className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl shadow-sm ${msg.sender.username === user?.username
