...
export default function NotificationsPopup({ isOpen, onClose }: NotificationsPopupProps) {
    ...
    const fetchNotifications = async () => {
        ...
        setIsLoading(true)
        try {
            const response = await fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/notifications/${user.username}`)
            ...
            <div className="absolute right-4 top-20 w-80 max-h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                    {isLoading ? (
                        ...
                        <div className="p-2">
                            <h4 className="text-sm font-medium text-gray-500 px-2 mb-3">Recent</h4>
                            <div className="space-y-2">
                                {notifications.map((notification) => (
                                    <Link key={notification.id} href={`/event/${notification.homefeedItem.id}`} className="block">
                                        <div
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-sm text-gray-600 mt-1">
                                                            <span className="font-medium">{notification.user.name}</span> {notification.type} this
                                                            item
                                                        </p>
                                                    </div>
