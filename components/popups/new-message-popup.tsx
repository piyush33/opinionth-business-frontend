...
export default function NewMessagePopup({ isOpen, onClose, onBackToInbox }: NewMessagePopupProps) {
    ...
    const fetchUniqueUsers = async (username: string) => {
        try {
            ...
            const [followersResponse, followingResponse] = await Promise.all([
                fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${username}/followers`, { headers }),
                fetch(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${username}/following`, { headers }),
            ])
            ...
            <div className="absolute right-4 top-20 w-96 h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-pink-50 flex-shrink-0">
                    <div className="flex items-center space-x-3">
                        ...
                        <h3 className="font-semibold text-gray-900">{selectedUser ? selectedUser.name : "New Message"}</h3>
                    </div>
                </div>
                ...
                {selectedUser ? (
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                        ...
                        <div className="flex-1 overflow-y-auto min-h-0">
                            <div className="p-2">
                                <h4 className="text-sm font-medium text-gray-500 px-2 mb-3">Suggested</h4>
                                <div className="space-y-1">
                                    {filteredUsers.map((user, index) => (
                                        ...
                                        >
                                            <div className="relative">
                                                {user.image ? <Image
                                                    src={user.image || "/placeholder.svg?height=44&width=44"}
                                                    alt={user.name}
                                                    width={44}
                                                    height={44}
                                ...
                                {filteredUsers.length === 0 && searchTerm && (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-gray-400 text-xs mt-1">Try searching with a different term</p>
                                    </div>
                                )}
                            </div>
                        </div>
