...
export default function Card({
    ...
    function extractReferences(text: string | undefined | null) {
        ...
        const users = [...new Set(matches.filter((m) => !/^@\d+$/.test(m)).map((m) => m.slice(1)))]
        const cards = [...new Set(matches.filter((m) => /^@\d+$/.test(m)).map((m) => Number.parseInt(m.slice(1))))]
        return { users, cards }
    }
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute inset-0 bg-white rounded-2xl border border-gray-400/60 shadow-sm transition-all duration-500 ease-out ${isHovered && !showActions
            ...
            {/* Third layer */}
            <div
                className={`absolute inset-0 bg-white rounded-2xl border border-gray-400/60 shadow-md transition-all duration-400 ease-out ${isHovered && !showActions
            ...
            {/* Second layer */}
            <div
                className={`absolute inset-0 bg-white rounded-2xl border border-gray-400/60 shadow-lg transition-all duration-300 ease-out ${isHovered && !showActions
            ...
            {/* Main card */}
            <div
                className={`relative bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden transition-all duration-300 ease-out hover:shadow-2xl  ${selected ? "ring-2 ring-purple-500 shadow-xl shadow-purple-100/50" : ""
                    } ${isHovered && !showActions ? "shadow-2xl border-gray-300 -translate-y-1" : ""}`}
                style={{ zIndex: 10 }}
                id={`card-${cardData?.id}`}
            >
                    ...
                    <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                        {picture ? (
                            ...
                            <div className="w-10 h-10 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-purple-400 to-blue-400 flex items-center justify-center flex-shrink-0">
                                <span className="text-white font-bold text-base sm:text-sm">{user.charAt(0).toUpperCase()}</span>
                            </div>
                        )}
                            ...
                            <div className="flex items-center space-x-2 min-w-0">
                                ...
                                {reposted && (
                                    <div className="flex items-center space-x-1 flex-shrink-0">
                                        ...
                                        <span className="text-xs text-green-600 font-medium hidden sm:inline">reposted</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="border-t border-gray-100 pt-4 mt-3">
                            <div className="flex items-center justify-start space-x-1 sm:space-x-3 mb-3">
                                <button
                                    ...
                                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full transition-all duration-200 flex items-center justify-center ${isLiked
                                ...
                                <button
                                    ...
                                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full transition-all duration-200 flex items-center justify-center ${isReposted
                                ...
                                <button
                                    ...
                                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full transition-all duration-200 flex items-center justify-center ${isSaved
