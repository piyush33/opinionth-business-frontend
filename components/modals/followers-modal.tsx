...
export default function FollowersModal({ isOpen, onClose, username, isOwnProfile = false }: FollowersModalProps) {
    ...
    const fetchFollowers = async () => {
        ...
        try {
            ...
            const response = await axios.get(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${username}/followers`, {
    ...
    const handleFollowToggle = async (follower: Follower) => {
        ...
        // Optimistic update
        setFollowers((prev) => prev.map((f) => (f.id === follower.id ? { ...f, isFollowing: !isCurrentlyFollowing } : f)))

        try {
            if (!isCurrentlyFollowing) {
                // Follow
                await axios.post(`https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${follower.username}/followers`, {
                ...
                const followingItem = followingResponse.data.find((item: any) => item.username === follower.username)
                ...
                if (followingItem) {
                    ...
                    const followersResponse = await axios.get(
                        `https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${follower.username}/followers`,
                    )
                    const followerItem = followersResponse.data.find((item: any) => item.username === user.username)

                    if (followerItem) {
                        await axios.delete(
                            `https://d3kv9nj5wp3sq6.cloudfront.net/profileusers/${follower.username}/followers/${followerItem.id}`,
                        )
                    }
                }
            }
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute right-4 top-20 w-96 max-h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-purple-50">
                    <div className="flex items-center space-x-3">
                        <Users className="w-6 h-6 text-blue-600" />
                        <h3 className="font-semibold text-gray-900 text-lg">{followers.length} Followers</h3>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    {isLoading ? (
                        ...
                        <div className="space-y-3">
                            {filteredFollowers.map((follower) => (
                                ...
                                >
                                    <button onClick={() => handleUserClick(follower)} className="flex items-center space-x-3 flex-1">
                                        <div className="relative">
                                            {follower.image ? <Image
                                                src={follower.image || "/placeholder.svg?height=48&width=48"}
                                                alt={follower.name}
                                                width={48}
                                                height={48}
                                        ...
                                        <div className="flex-1 text-left">
                                            <p className="font-medium text-gray-900 text-sm">{follower.name}</p>
                                    ...
                                    {!isOwnProfile && follower.username !== user?.username && (
                                        <button
                                            ...
                                            className={`flex items-center space-x-1 px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${follower.isFollowing
                    ...
                    {filteredFollowers.length === 0 && !isLoading && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-500 text-sm">{searchTerm ? "No followers found" : "No followers yet"}</p>
                        </div>
                    )}
                </div>
            </div>
